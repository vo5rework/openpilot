...
static void tesla_legacy_rx_hook(const CANPacket_t *msg) {
  ...
  const bool use_stalk_for_controls_allowed = tesla_legacy_op_stalk_enable || (tesla_legacy_use_stalk_controls_allowed || (!has_ap_hardware) || tesla_legacy_op_autopilot_disabled);
...
static bool tesla_legacy_tx_hook(const CANPacket_t *msg) {
  ...
  // OP_STALK_0x659_EDGE_CARRIER_V3
  ...
  if (msg->addr == 0x659U) {
    ...
    // In XNOR, safety_tick can clear controls_allowed asynchronously; ensure stalk edges always re-latch.
    if (stalk_main && (tesla_legacy_op_autopilot_disabled || tesla_legacy_op_stalk_enable)) {
      controls_allowed = true;
      cruise_engaged_prev = true;
