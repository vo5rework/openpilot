...
class LongController:
  ...
  _PLANNER_DRAG_MARGIN_MS = 1.0
  _PLANNER_BELOW_EGO_MARGIN_MS = 0.8
  _STRONG_DECEL_ATARGET_MS2 = -0.7
  _LEAD_OPENING_VREL_MS = 0.35
  _LEAD_RELEASE_TIME_GAP_S = 1.5
  _LEAD_RELEASE_MIN_GAP_M = 30.0
  ...
  def _lead_release_ready(self, *, v_ego_ms: float) -> bool:
    if (not self._lead_present) or float(self._lead_drel) <= 0.0:
      return False

    release_gap_m = min(
      100.0,
      max(float(self._LEAD_RELEASE_MIN_GAP_M), float(v_ego_ms) * float(self._LEAD_RELEASE_TIME_GAP_S)),
    )
    return bool(
      float(self._lead_vrel) > float(self._LEAD_OPENING_VREL_MS)
      and float(self._lead_drel) >= float(release_gap_m)
    )
  ...
  def _lead_is_constraining(self, *, base_target_ms: float, v_ego_ms: float) -> bool:
    ...
    if self._lead_release_ready(v_ego_ms=float(v_ego_ms)):
      return False
  ...
  def _planner_drag_reasons(self, *, base_target_ms: float, planner_ms: float, v_ego_ms: float) -> list[str]:
    ...
    lead_release_ready = self._lead_release_ready(v_ego_ms=float(v_ego_ms))
    lp_lead_constraining = bool(self._lp_has_lead) and (not lead_release_ready) and (
      lead_constraining
    ...
    reasons: list[str] = []
    if lead_constraining and not lead_release_ready:
      reasons.append("lead")
    if lp_lead_constraining:
      reasons.append("lp_hasLead")
    if (float(self._lp_a_target) <= float(self._STRONG_DECEL_ATARGET_MS2)) and not lead_release_ready:
