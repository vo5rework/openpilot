...
Patch170 keeps the stable patch158 owner split and only narrows the no-lead
curve path:

- lead-following and lead-gap behavior stay on the existing planner-tail path
- no-lead curve control still uses planner-based curve entry, but with a shorter
  near-horizon window so small/far-ahead dips do not over-own the target
- highway curve entry now requires a larger slowdown request before latching
- active curve-hold ignores tiny additional drops to avoid 1-3 mph nibbles
- mapd is used only as a release hint for an already-active no-lead curve hold,
  so speed can recover cleanly after the bend without giving mapd startup / lead ownership
- slight dips below Tesla minimum cruise are still clamped to minimum instead of
  needlessly cancelling ACC
...
class LongController:
  ...
  _LEAD_HOLD_PERSIST_MS = 1200
  _LEAD_HOLD_RELEASE_MARGIN_MS = 1.0 * CV.MPH_TO_MS
  ...
  def _lead_hold_active_now(self, *, now_ms: int, base_target_ms: float, planner_ms: float) -> bool:
