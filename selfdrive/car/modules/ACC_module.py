...
- make automatic acceleration calmer by preferring 1-step RES presses, while allowing slightly quicker recovery when a lead opens up
"""
  ...
  def update(
    ...
    *,
    ...
    if float(desired_speed_ms) < float(self.MIN_CRUISE_SPEED_MS):
      ...
      accel_ready = self._no_automated_action_for(now_ms=now_ms, milliseconds=self._AUTO_COOLDOWN_ACCEL_MS)
      full_press_gap_kph = 1.25 * float(full_kph)
      full_press_available_kph = 1.25 * float(full_kph)
      if accel_ready and speed_offset_kph >= float(full_press_gap_kph) and float(full_press_available_kph) < float(available_speed_kph):
